...
// 워커를 통한 파일 서빙 URL
//  - 다운로드: 원본 파일명 유지(Content-Disposition: attachment)
//  - 미리보기: 인라인 + 텍스트 charset=utf-8 (한글 깨짐 방지)
export function fileDownloadUrl(key, name) {
  return `${WORKER_URL}/file?key=${encodeURIComponent(key)}&name=${encodeURIComponent(name || 'file')}&dl=1`
}
export function fileViewUrl(key, name) {
  return `${WORKER_URL}/file?key=${encodeURIComponent(key)}&name=${encodeURIComponent(name || 'file')}`
}
